<template>
  <section class="todoapp">
    <header class="header">
      <v-btn flat depressed small class="adding-todo" type="box" @click.prevent="AddTodo">
        Ajouter
      </v-btn>
      ...
      <ul class="todo-list" id="list">
        <li class="todo" v-for="(list, index) in $store.state.lists" v-if="list.text" :key="list.id" :class="{completed: list.done}">
          <div class="view">
            ...
            <button type="checkbox" class="destroy" @click.prevent="DeleteTodo(index)">
            </button>
          </div>
        </li>
      </ul>
      <footer class="footer" v-show="$store.state.count">
        <div>
          <span class="todo-count"> <strong>{{ DoneTodo }}</strong>
            : Nombre de Todo
          </span>
          <button class="clear-completed" type="box" @click.prevent="DeleteAllTodo">
            Tout Supprimer
          </button>
        </div>
      </footer>
